...
// Dumps just the TOP 10% of the dream-card name-strip region from each slot.
// Region: x∈[0.0608, 0.20], y∈[0, 0.06] of the card. Saves as
// `topstrip_<screenshot>_slot<N>.png`.
...
const TOP_STRIP_REGION = { x: 0.0608, y: 0.00, width: 0.1392, height: 0.06 };

const CASES = [
  { ss: 'fengxuround6.png',  slot: 6, label: 'r6_slot6_轰雷掣电_truthP2' },
  { ss: 'fengxuround7.png',  slot: 3, label: 'r7_slot3_星轨推衍_truthP2' },
  { ss: 'fengxuround7.png',  slot: 0, label: 'r7_slot0_乾卦_truthP3' },
  { ss: 'fengxuround7.png',  slot: 2, label: 'r7_slot2_轰雷掣电_truthP3' },
  { ss: 'fengxuround15.png', slot: 0, label: 'r15_slot0_两仪阵_truthP5' },
  { ss: 'fengxuround15.png', slot: 5, label: 'r15_slot5_星轨推衍_truthP5' },
];
...
for (const c of CASES) {
  ...
  // Slot rect (the full dream card region).
  ...
  // Top 10% of name strip (within the slot).
  const tx = slotX + Math.round(TOP_STRIP_REGION.x * slotW);
  const ty = slotY + Math.round(TOP_STRIP_REGION.y * slotH);
  const tw = Math.max(1, Math.round(TOP_STRIP_REGION.width  * slotW));
  const th = Math.max(1, Math.round(TOP_STRIP_REGION.height * slotH));
  ...
  const file = path.join(__dirname, `topstrip_${c.label}.png`);
  fs.writeFileSync(file, encodePng(out, tw, th));
  console.log(`${c.label}: ${tw}x${th} → ${file}`);
}
